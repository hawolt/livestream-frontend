...
There is no separate public clip page. A created clip plays at `/<channel>/clip/<code>`, served with the channel viewer's own `live.html` template (the backend splices clip `og`/`twitter` meta into it server-side for that route); see "Clip playback mode" above.
...
| `GET /api/live/clips/{code}` (absolute path) | clip editor, clip playback mode | `{id, channel, title, status, phase, queuePosition, durationMs, createdAt, url, thumbnailUrl, pageUrl}`, `phase`/`queuePosition` present while `status` is `processing`, 404 for no clip |
