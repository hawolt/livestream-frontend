...
- **Loading**: `GET /api/live/explore` runs immediately and every 20 s while the page is visible. Failures retry after 5 s; the first failure shows a retrying state, while later failures retain the last successful data. Requests are serialized, aborted on page hide, and protected by a generation check. The response provides `streams` (`username`, `title`, `category`, `categoryId`, `viewers`, optional per-stream `mediaBase`), `categories` (`id`, `name`, `liveStreamCount`, `viewerCount`), and a default `mediaBase`.
...
- **Category mode** renders cached category cards (plus a synthetic "No category" card when uncategorized streams exist), sorted by summed viewers; stable cards update in place so polling does not discard keyboard focus. Clicking one drills into its stream list.
...
`explore.ts` is the entry: it wires the mode/back buttons, the grid's category click delegation, `popstate`, visibility lifecycle, and boot. The rest lives in `src/explore/`: `context.ts` (shared state and types, `isFramed`), `dom.ts` (element lookups), `thumb-url.ts` and `thumbnail-minute.ts` (thumbnail URL building), `stream-cards.ts` (card cache, grid rendering), `preview.ts` (the hover preview overlay and its `postMessage` handling), `categories.ts` (category grid and drill-down), `url-state.ts` (pure URL/`ViewState` conversion), `render.ts` (mode dispatch and `pushState` navigation), and `poll.ts` plus `poll-loop.ts` (the visible-page request loop).

## Ads (`src/ads.ts`)

Used by the channel viewer's chat column. `loadAds("chat")` fetches `GET /api/live/spots?slot=chat` as a plain `fetch`, attaching a `Bearer` header from the `dash_token` in `sessionStorage` when it decodes as a valid session (`session-token.ts`). Any non-OK response, thrown error, or non-array `ads` field resolves to an empty list. `renderAdSlot(container, ads)` clears the container and, for a non-empty list, builds one `<a href="/api/live/spots/visit/<id>" target="_blank" rel="noopener noreferrer nofollow sponsored">` containing an image and an `Anzeige` label entirely with DOM methods. An empty list renders nothing.
...
Reconnects: 5 s normally, 30 s after a flood kill (close 4400) or while banned, and a "Chat is open in too many windows." message plus 30 s on close reason `session-limit`. Close codes 4400/4401/4402/1009 render explanatory system lines. A new connection resets all member, role, subscriber, and pin state. Every socket callback checks that it still belongs to the current connection.
...
A reduced viewer for `/embed/<username>`: same generation-guarded transport machinery (WS+MSE primary, native HLS fallback, channel API for `mediaBase`, captcha, viewer-id beacon on HLS, backoff retries, abortable metadata boot, pagehide/pageshow handling) but no chat, no seek bar, no health timer, a 30 s buffer, and a simple offline poster. Autoplays muted; a click on the unmute button or anywhere on the stage unmutes.
...
## Follow alert overlay (`src/follow-alerts.ts`)

`/alerts/<username>` connects to `/ws/events` with a scoped overlay token and renders follow cards sequentially. Generated dashboard URLs put the token in `#token=` so it is not sent in HTTP requests or referrers. Legacy `?token=` URLs still work and are immediately migrated in browser history. The queue is capped at 20 events and drops entries older than two minutes. `size`, `duration`, and `demo=1` remain query parameters.
...
- **Register** (`src/register.ts`): same already-signed-in redirect logic. The hCaptcha script loads only after those checks determine that registration should continue. Submission runs the invisible widget (`hcaptcha.execute`, 20 s watchdog, reset on error/expiry); the resulting token is sent with `{username, password, email, captchaToken}` to `POST /api/auth/register`. Success reloads the first-party registration module with the bearer and a one-time random state in the fragment. The fresh document immediately scrubs the fragment, rejects unbound completion URLs, verifies the bearer before storing it, and redirects without loading hCaptcha.
- **Verify** (`src/verify.ts`): accepts `#token=` and legacy `?token=`, immediately removes either from the visible URL, calls `GET /api/auth/verify?token=`, and on `{ok}` links and auto-redirects (2.5 s) to the dashboard host matching the returned account `kind`.
- **Reset password** (`src/reset-password.ts`): accepts `#token=` and legacy `?token=`, immediately removes either from the visible URL, validates the two password fields match, posts `{token, newPassword}` to `POST /api/auth/reset-password`, and on success shows a done section and redirects to `/login`.
...
User dashboards also connect to `/ws/events`. A subscription event refreshes `/auth/me`; a request revision prevents older refreshes from overwriting newer entitlements. The full local tab metadata and Studio availability are compared, so label, group, pane, and Studio-only changes rebuild navigation. Revoking the active tab deactivates its pane and selects the first remaining tab, or shows an empty state when none remain.
...
- **Alertbox** (`alertbox.ts`): loads and can rotate the follow-overlay credential, builds fragment-token OBS URLs, sends test alerts, and renders a debounced demo preview. `deactivate()` blanks the iframe.
- **Activity** (`activity.ts`): merges the recent-follows snapshot with live `/ws/events` updates, shows follower/viewer counts, edits current stream metadata, and loads a chat popout for the current channel. Activation generations discard late responses; deactivation closes the event socket and unloads the chat iframe.
- **Channel Profile** (`profile.ts`): edits bio and external links plus avatar/banner upload and removal. Destructive and save controls remain disabled until the current profile loads successfully.
...
Third-party: `7tv.io` (emote sets), `challenges.cloudflare.com` (Turnstile script), `js.hcaptcha.com` (registration captcha script, loaded by `register.ts` after session checks).
