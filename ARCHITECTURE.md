...
- **Clips** (`clips.ts`, group Account): `GET /api/clips` lists the caller's own clips (`Clip[]`, thumbnail/title/channel/created/duration/status). Each row gets an Open link to `/<channel>/clip/<code>` in a new tab, a Copy link button (`src/clipboard.ts`), and a confirm-gated Delete button calling `DELETE /api/clips/{id}` and removing the row on success. A processing or failed clip shows a status chip; an empty list shows a plain message.
...
| `GET /clips`, `DELETE /clips/{id}` | clips tab | `Bearer` auth; list gives `{clips: Clip[]}` (`id, channel, title, status, url, thumbnailUrl, durationMs, createdAt`, the caller's own clips only), delete is status-only (`204`) |
