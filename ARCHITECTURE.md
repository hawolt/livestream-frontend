...
### Clip creator (WS only)

Clipping no longer captures or uploads any media; the server keeps a rolling source-quality fragment ring per live stream, and the client only reserves a slice of it, previews from its own frozen playback buffer, and submits timestamps.

A clip button appears in the controls once `transportKind` is `ws` and the live video's buffered range covers at least 10 s, next to the cinema and fullscreen buttons. Signed out, it opens the login modal with a `clip` intent; signed in (a valid `dash_token` in `sessionStorage`, `src/session-token.ts`), it opens the clip editor (`src/live/clip/editor.ts`), an overlay docked to the bottom of the stage so the live video itself stays visible above it, since the preview is that same video.

Opening the editor freezes playback instead of capturing anything: `freezeForClip()` (`src/live/player/lifecycle.ts`) pauses the video, stops the 500 ms chase timer and the health timer, and closes the WS socket, all without touching the `MediaSource`/`SourceBuffer` or the video's `src`, so the buffered range the viewer was already watching stays intact and on screen. `pump()`/`pruneBuffer()` (`src/live/player/mse.ts`) also bail out while frozen as a second guard, though with the socket closed no new frames arrive to append or prune anyway. The video's buffered end at the moment of freezing is recorded as the freeze edge, and every later step (the selectable range, the two-handle selector, the submitted span) is measured relative to it. Closing the editor (submit, discard, or Escape) calls `resumeFromClip()`, which restarts the transport through the normal `beginTransport()` restart path, the same one quality switches use, so the player reconnects and snaps back to the live edge.

The server owns the fragment ring; the client only pins a slice of it. Opening the editor `POST`s `/api/main/v1/clips/pin?channel=<username>` with an `Authorization: Bearer <dash_token>` header and gets back `{pin, nowMs, windowMs}`, shown as a brief loading state while it is in flight. A `404` means the channel cannot be clipped right now and a `429` means too many pin requests; both end the session in a dead-end state with only a close button. A `401` closes the editor (which resumes playback) and reopens the login modal. While the editor stays open with a pin, `src/live/clip/pin.ts` renews it every 5 minutes via `POST /api/main/v1/clips/pin/renew?channel=&pin=`; a failed renewal ends the session the same way a `404` does. Closing the editor before submitting best-effort `DELETE`s the pin (`/api/main/v1/clips/pin?channel=&pin=`); submitting keeps it, since the server consumes it as part of clip creation.

The two-handle range selector (`src/live/clip/span.ts`, styled after the seek bar) spans the intersection of the video's buffered range at freeze time and `windowMs` behind the freeze edge (`selectableRange()`), clamped between 3 s and 30 s (`clampClipSpan()`). Dragging a handle seeks the main video element itself to that position, since the frozen buffer only exists on that one element and there is nothing left to build a separate preview from. `behindMsFromSelection()` converts the selection, kept internally in local buffered seconds, into the pair the server wants: `startBehindMs`/`endBehindMs`, each the freeze edge minus the selection bound in milliseconds, so `startBehindMs > endBehindMs >= 0`.

Submitting is a bodyless `POST /api/main/v1/clips?channel=&title=&pin=&startBehindMs=&endBehindMs=` with the `Bearer` header; a `202` gives `{id, status}` and the editor shows a plain submitting state, since there is no upload to show progress for. A `409`, `410`, or `422` is treated as a failed clip; a `429` returns to the form with a rate-limit message and keeps the pin; a `503` ends the session as feature-unavailable. On success the editor polls `GET /api/live/clips/{id}` (plain fetch, visibility-gated every 2 s, the same pattern as `src/explore/poll.ts`) while `status` is `processing`, then shows a link to `/clip/{id}` with a copy button on `ready`, or the failure reason on `failed`. While still `processing`, the response optionally carries `phase` (the encoder's own `queued`/`encoding`/`ready`/`failed` status) and `queuePosition` (jobs ahead of this one); the shared pure `clipProcessingMessage()` (`src/clip-processing-message.ts`) turns those into "In queue, N ahead" while queued behind others, "Encoding" once encoding starts (or `queuePosition` reaches 0), or the plain generic processing text when the fields are absent (an older backend, or a job still sitting in the pre-dispatch gap).
...
## Clip page (`src/clip.ts`)

`/clip/<id>`: a public, unauthenticated playback page for a created clip. `src/clip/id.ts` takes the id from the second path segment and validates it against `^[0-9a-f]{16}$`; anything else is a terminal not-found state, and no request is made. `initSiteNav(null)` renders the standard site-nav header; the rest of the page is a single state area plus a body (title, a link to `/<channel>`, the creation date, and a `video` element), toggled between a loading, not-found, processing, ready, or failed state (`src/clip/render.ts`).

A valid id fetches `GET /api/live/clips/{id}` (plain fetch, absolute path, same as the viewer's channel lookup) via `src/clip/api.ts`. A `404` is the not-found state; any other non-OK or network failure is a load-error state. On success, a `status` of `processing` shows a processing message (the same `phase`/`queuePosition`-aware `clipProcessingMessage()` the clip editor uses, see above) and starts a visibility-gated poll of the same endpoint every 2 s (`src/clip/poll.ts`, the same pattern as `src/explore/poll.ts` and the clip editor's own poll), stopping once the status leaves `processing`. `ready` shows the `video` element with `controls` pointed at the response's `url`; `failed` shows the response's `error` message.
...
| `GET /api/live/clips/{id}` (absolute path) | clip editor, clip page | `{id, status, title, channel, createdAt, url, error}` plus optional `phase`/`queuePosition` while `status` is `processing`, 404 for no clip |
