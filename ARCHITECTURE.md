...
Clip creation happens on its own page in a new tab; the viewer only exposes the entry point. A clip button appears in the controls once `transportKind` is `ws` and the live video's buffered range covers at least 10 s, next to the cinema and fullscreen buttons (`src/live/clip/button.ts`). Its click handler calls `window.open("/clip/create?channel=<username>", "_blank")` synchronously, before any `await`, so the browser does not treat it as a blocked popup. Signed out (no valid `dash_token` in `sessionStorage`, `src/session-token.ts`), it opens the login modal with a `clip` intent instead; on successful login the modal (`src/live/login-modal.ts`) opens the same new tab. Live playback is never touched by any of this: there is no freeze machinery, no pin lifecycle, and no editor overlay in the viewer.
...
`/wiki` (Apache rewrite to `wiki.html`) is the developer documentation page linked as "API" in the navbar kebab. All content is static markup in `wiki.html`: one `<section data-topic data-title>` per topic inside `#wiki-content`, currently **Chat bots** (IRC endpoints, token auth, tags, commands, limits), **Public API** (token creation in the dashboard Settings card, Bearer auth, the 120/min per-token rate limit and `X-RateLimit-Remaining`, every `/api/public/v1` endpoint with params, curl, example response and errors, and the cursor pagination contract), **Sign in with itzon** (client registration, the authorize URL, consent redirect, code-for-token exchange, userinfo, the OAuth error codes, 60 s single-use codes and 1 h tokens), and **OBS setup** (ingest URL + stream key from the Stream tab, OBS service/encoder walkthrough with the 10000 kbps declared cap, 2 s keyframe interval, resolution/fps baselines and purchasable ceilings, go-live checklist, common failures).
...
`/oauth/authorize` (Apache rewrite to `oauth-authorize.html`; `oauth` is reserved as a username) is the "Sign in with itzon" consent page a third-party app sends its users to with `?client_id=&redirect_uri=&response_type=code&scope=identity&state=`. The query is parsed and validated purely client-side first (`parseAuthorizeParams()`, `src/oauth-authorize/params.ts`, tested: 32-hex client id, absolute http(s) fragment-free redirect URI, `response_type` exactly `code`, scope exactly `identity` when present, state capped at 512 chars); an unparseable request renders a terminal error with no network call and never redirects anywhere.

The session boots the way the clip editor does (`src/oauth-authorize/session.ts`: stored `dash_token`, else one `GET /api/auth/session` cookie bootstrap); signed out redirects to `/login?return=<this page's URL>` and lands back here after login. It then calls `GET /oauth/authorize` (under `API_BASE`, Bearer) which validates the client and exact redirect match server-side and returns `{app, scope, username}`; a 401 clears the stored token and round-trips through login, any other failure is terminal (again, no redirect, since the redirect URI was never validated). Success renders "APPNAME wants to verify your itzon identity" with the `identity` scope description and Approve/Deny buttons. Approve posts `{clientId, redirectUri, scope, state}` to `POST /oauth/consent` (Bearer) and navigates to the returned `{redirect}` (the registered redirect URI carrying `code` + `state`); Deny navigates to `buildDenyRedirect()` (`redirect_uri?error=access_denied&state=`, pure and tested) - safe to do client-side because the server already confirmed the redirect URI is registered.
...
4. Granted tabs are filtered to those with an entry in `TAB_LOADERS` (`stream`, `discord`, `stream-manager`, `chatbox`, `alertbox`, `stream-health`, `stream-summary`, `activity`, `settings`, `channel-profile`, `clips`, `subscription`). Granted tabs without a local loader are not rendered as tabs; their presence makes the sidebar (and the mobile hamburger) show a single external link instead.
...
| `POST /clips/pin?channel=`, `POST /clips/pin/renew?channel=&pin=`, `DELETE /clips/pin?channel=&pin=` | clip editor | `Bearer` auth; pin gives `{pin, nowMs, windowMs}` (`nowMs` is the media-timeline ms of the ring's newest fragment at pin time, `windowMs` how far back it reaches; `404` unclippable, `429` rate-limited), renew and release are status-only |
...
| `POST /settings/api-tokens`, `GET /settings/api-tokens`, `DELETE /settings/api-tokens/{id}` | settings | create gives `ApiTokenCreated` (`{id, token, label, prefix, createdAt}`, the plaintext `token` appears exactly once here); list gives `{tokens: ApiTokenInfo[]}` (`{id, label, prefix, createdAt, lastUsedAt}`); delete is status-only (`204`), `404` for a token the caller does not own |
...
| `POST /oauth/consent` | OAuth consent page | `Bearer` auth, body `{clientId, redirectUri, scope, state}`; `{redirect}` = the registered redirect URI carrying the single-use `code` + `state` |
