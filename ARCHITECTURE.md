...
- **Live channel rail**: polls `GET /api/live/explore` every 10 s while visible, sorts live channels by viewer count, highlights the current channel, and links directly to every live viewer page. Its collapsed state persists under `live-channel-rail-collapsed`; narrower horizontal layouts keep a compact avatar-and-name rail, while vertical, cinema, chat fullscreen, chat popout, fullscreen-video, and browse modes pause polling until the rail becomes visible again.
