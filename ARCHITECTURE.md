...
A path matching the clip route puts `live.ts` into clip mode (`bootClipMode()`, `src/live/clip/mode.ts`) instead of the normal viewer boot: `document.body` gets a `clip-mode` class (CSS hides the custom `.live-controls`, the channel rail, viewer count, uptime, and follow controls), the chat column collapses without touching the persisted `live-chat-collapsed` preference, and neither `startChat()` nor `beginTransport()` is ever called, so no IRC connection, WS/HLS transport, health timer, or retry backoff is registered for this mode.

It fetches `GET /api/live/clips/{code}` (`src/clip-status-poll.ts`, shared with the clip editor's post-create polling) and checks the response's `channel` against the URL channel, case-insensitively; a mismatch, a 404, or an unparseable response renders a not-found state. The clip's title and channel populate the existing info bar (`#live-title-text`, `#live-name`); state messages (loading, not-found, processing, failed) render into the existing `#live-poster` element, reused as-is rather than through any transport-lifecycle helper. A `status` of `processing` polls every 2 s (visibility-gated) showing the same `phase`/`queuePosition`-aware `clipProcessingMessage()` (`src/clip-processing-message.ts`) the editor uses; `ready` sets the stage `video`'s `src` to the response's `url`, its `poster` to `thumbnailUrl` when present, and turns on native `controls` (the custom `.live-controls` overlay stays hidden throughout clip mode).

Independently, it fetches `GET /api/live/channel/{channel}` for channel chrome; when that response indicates the channel is currently live, a prominent accent-colored "Watch live now" button (`#live-clip-watch-live`, styled unmissable, top-center of the stage) links to `/{channel}`.
...
| `GET /api/live/channel/<user>` (absolute path) | viewer, embed, clip playback mode | `{title, category, categoryId, mediaBase, emoteTwitchId}`, 404 for no channel; clip playback mode additionally reads an optional `live` flag to decide whether to show "Watch live now" |
