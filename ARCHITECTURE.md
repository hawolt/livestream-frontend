...
With a pin in hand, it creates a `MediaSource`, picks the first supported `video/mp4` codec string from a small candidate list (the pin/media response carries no codec header), and streams `GET /api/main/v1/clips/pin/media?channel=&pin=` (`src/clip-editor/media.ts`) via `response.body.getReader()`, appending each chunk to the `SourceBuffer` as it arrives regardless of chunk boundaries. A load overlay over the video shows progress as the `SourceBuffer`'s buffered end (converted from `X-Media-Start-Ms`) advances toward `X-Window-Ms`. The response headers `X-Now-Ms`, `X-Window-Ms`, and `X-Media-Start-Ms` become `state.nowMs`/`windowMs`/`mediaStartMs`; the media timeline is the source timeline, so `video.currentTime * 1000` reads directly as an absolute position on that same timeline, no offset math needed. Once fully buffered, `mediaSource.endOfStream()` is called and the editor becomes interactive.
...
| `GET /clips/pin/media?channel=&pin=` | clip editor | streams the pinned window as raw progressive fMP4 bytes (init segment then fragments, appendable to MSE in arbitrary chunks); headers `X-Now-Ms`, `X-Window-Ms`, `X-Media-Start-Ms` (media-timeline ms of the first fragment) |
