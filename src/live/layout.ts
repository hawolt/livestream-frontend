import { btnCinema, btnLayoutToggle, channelRailEl, chatEl, chatHeadActionsEl, chatHeadEl, mainEl, page, uptimeEl, video } from "./dom.ts";
...
export function syncLayout(): void {
    ...
    if (isVertical) {
        uptimeEl.after(chatHeadActionsEl);
...
export function fitChat(): void {
    ...
    const availH = window.innerHeight - STAGE_CHROME_PX;
    if (availH <= 0) {
        chatEl.style.width = "";
        return;
    }
    ...
    const ideal = mainEl.clientWidth - railWidth - availH * aspect;
    const want = Math.round(Math.min(CHAT_MAX_PX, Math.max(CHAT_BASE_PX, ideal)));
